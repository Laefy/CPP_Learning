...
```cpp
Person clone = batman;
std::cout << "Person named '" << clone.get_full_name() << "' is " << clone.get_age() << " years old." << std::endl;
```
{{% /expand %}}
...
Afin de réaliser l'affichage en bonne-et-due forme de votre objet, vous allez implémenter une nouvelle version de l'opérateur `<<`.
...
Pour le type de retour, vous devez utiliser `std::ostream&`, car c'est ce qui vous permettra d'utiliser `<<` plusieurs fois dans la même instruction.
...
Lancez le programme. Vous devriez constater que l'âge de `assigned_clone` est de 23 ans.

Pour être cohérent avec ce que nous avons fait précédemment, nous allons dire que l'âge d'une instance de `Person` est toujours initialisé 0 lors de sa construction et qu'il ne peut être modifié que lors d'un appel à `wait`.
L'**implémentation par défaut de l'opérateur d'assignation** ne respecte pas cette proposition, puisqu'elle modifie l'âge de notre instance.
